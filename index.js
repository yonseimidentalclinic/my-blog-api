...
// --- [수정] 데이터베이스 연결 및 초기화 ---
// Pool 객체 생성부터 try...catch로 감싸서 연결 자체의 에러를 잡습니다.
let pool;
try {
    console.log(">>> 데이터베이스 연결 풀(Pool) 생성을 시도합니다...");
    pool = new Pool({
    ...
    console.log(">>> 데이터베이스 연결 풀(Pool) 생성 성공.");
} catch (err) {
    console.error(">>> 치명적 에러: 데이터베이스 연결 풀(Pool) 생성 실패.", err);
    process.exit(1);
}
...
const initializeDatabase = async () => {
    let client;
    try {
        console.log(">>> 데이터베이스에 연결하여 테이블 초기화를 시작합니다...");
        client = await pool.connect();
        console.log(">>> 데이터베이스 연결 성공. 테이블 확인 및 생성을 시작합니다.");
        ...
        `);
        console.log('>>> 모든 데이터베이스 테이블이 성공적으로 확인 및 생성되었습니다.');
    } catch (err) {
        console.error('>>> 치명적 에러: 데이터베이스 초기화 실패.', err);
        process.exit(1);
    } finally {
